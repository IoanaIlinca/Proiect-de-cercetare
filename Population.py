...
class Chromeosome:
    def __init__(self, path, eval):
        self.path = path
        self.fitness = path.__len__() # add score
...
class Population:
    ...
    def __init__(self, world):
        ...
        self.shortestPaths = PriorityQueue()
        self.shortestPaths.put((self.shortestPath.__len__(), self.shortestPath))
    ...
    def drawTexts(self, child):
        ...
        cLen = child.__len__()
        if cLen == self.shortestPath.__len__() and ((cLen, child) not in item for item in self.shortestPaths.queue):
            try:
                self.shortestPaths.put((cLen, child))
        ...
        self.text = Text(
            ...
            "Current path length: " + str(cLen) + "  Best path detected: " + str(self.shortestPath.__len__())
        )
        ...
        for i in indices:
            for c in range(1, self.population[i].__len__() - 1):
                ...
                if r <= m:
                    ...
                    # Intersection
                    inters = [p for p in preNs if
                              self.world.cheackPath(postNs, p) and not (p.x == ns.x and p.y == ns.y)]
                    if inters.__len__() > 0:
