...
class World:
    ...
    def __init__(self):
        ...
        #  Init version 1
        startX = random.randint(0, self._width - 1)
        startY = random.randint(0, self._width - 1)
        while (startX, startY) in self.obstacles:
            startX = random.randint(0, self._width - 1)
            startY = random.randint(0, self._width - 1)
        self._startPoint = (startX, startY)

        stopX = random.randint(0, self._width - 1)
        stopY = random.randint(0, self._width - 1)
        while (startX, startY) in self.obstacles:
            stopX = random.randint(0, self._width - 1)
            stopY = random.randint(0, self._width - 1)
        self._startPoint = (startX, startY)
        self._endPoint = (stopX, stopY)
    ...
    def drawMap(self, window=None):
        for x in range(self._width):
            for y in range(self._height):
                self.drawCell(Point(x, y), self._world[x][y], window=window)

        self.drawPoint(Point(self._startPoint[0], self._startPoint[1]),
                       self._world[self._startPoint[0]][self._startPoint[1]], window=window)
        self.drawPoint(Point(self._endPoint[0], self._endPoint[1]),
                       self._world[self._endPoint[0]][self._endPoint[1]], window=window)
    ...
    def drawCell(self, p, type, window=None):
        if window is None:
            window = self.win
    ...
    def drawPoint(self, p, type, window=None):
        if window is None:
            window = self.win
        p1 = Point(p.x * self._cellSize + self._drawOffset, p.y * self._cellSize + self._drawOffset)
        p2 = Point(p1.x + self._cellSize, p1.y + self._cellSize)
    ...
    def drawLine(self, p1, p2, color=None, window=None):
        if window is None:
            window = self.win
        p_1 = Point(self._drawOffset + p1.x * self._cellSize + .5 * self._cellSize,
                    self._drawOffset + p1.y * self._cellSize + .5 * self._cellSize)
        p_2 = Point(self._drawOffset + p2.x * self._cellSize + .5 * self._cellSize,
                    self._drawOffset + p2.y * self._cellSize + .5 * self._cellSize)
        l = Line(p_1, p_2)
        l.setWidth(10)
        if color is None:
    ...
    def drawPath(self, path, joint=-1, final=False, window=None):
        p1 = None
        p2 = None
        ls = []
        ...
        for i, p in enumerate(path):
            if p1 is None:
                ...
                if joint > -1:
                    if final:
                        print("hey")
                        ls.append(self.drawLine(p1, p2, self.colorRGB(0.3, 0.7, 0.85), window))
                    elif joint < i:
                        ls.append(self.drawLine(p1, p2, self.colorRGB(0.3, 0.1, 0.85), window))
                    else:
                        ls.append(self.drawLine(p1, p2, self.colorRGB(0.2, 0., 0.5), window))
                else:
                    ls.append(self.drawLine(p1, p2, self.colorRGB(0.9, 0.1, 0), window))
                p1 = p2
                p2 = None
        ...
        while True:
            ...
            if self.population.geneticAlgorithm(draw=draw):
                ...
                for index in range(0, 5):
                    self.windows.append(GraphWin("Path " + str(index), self._windowWidth, self._windowHeight))

                for counter in range(0, 5):
                    path = self.population.shortestPaths.get()[1]
                    self.windows[counter].setBackground(self.backgroundColor)
                    try:
                        self.drawMap(self.windows[counter])
                    except Exception as ex:
                        print(ex)
                    self.drawPath(path, window=self.windows[counter])
                    print(path)
                '''
                try:
                    counter = 0
                    result = ""
                    while counter < 20 and not self.population.shortestPaths.empty():
                        path = self.population.shortestPaths.get()[1]
                        if self.population.shortestPaths.empty():
                            print("empty")
                            break
                        result += str(path) + "\n"
                        counter += 1

                    path = "files/file_" + str(datetime.now().strftime("%H-%M-%S")) + ".txt"
                    f = open(str(path), "w")
                    f.write(result)
                    f.close()
                    print(result)
